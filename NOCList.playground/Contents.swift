...
func totalNumOfCompromisedAgents() -> Int {
    var totalNumber = 0
    for agent in agents {
...
print(totalNumOfCompromisedAgents())
...
func findHighRisk() {
    for agent in agents{
        if agent.accessLevel >= 8{
            agent.compromised ? print("\(agent.realName), level: \(agent.accessLevel) **WARNING** **COMPROMISED**") : print("\(agent.realName), level: \(agent.accessLevel)")
        }
    }
}
...
func findLevelGroup(){
    var numOfLowAgents = 0
    var numOfMidAgents = 0
    var numOfHighAgents = 0
    ...
    for agent in agents{
        ...
        case 8...:
            numOfHighAgents += 1
        default:
            print("\(agent.coverName)'s access level is invalid")
        }
    }
    print("\(numOfLowAgents) low level agents, \(numOfMidAgents) mid level agents, and \(numOfHighAgents) high level agents")
}
